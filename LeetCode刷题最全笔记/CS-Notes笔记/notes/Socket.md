...
当 ==epoll_wait()== 检测到==描述符事件==到达时，将此事件通知进程，==进程可以不立即处理该事件==，下次调用 ==epoll_wait() 会再次通知进程==。是默认的一种模式，并且同时支持 Blocking 和 No-Blocking。
...
需要监控的==描述符状态==变化多，而且都是==非常短暂==的，也==没有必要使用 epoll==。因为 ==epoll 中的所有描述符都存储在内核中，造成每次需要对描述符的状态改变都需要通过 epoll_ctl() 进行系统调用，频繁系统调用降低效率。并且 epoll 的描述符存储在内核，不容易调试。==
