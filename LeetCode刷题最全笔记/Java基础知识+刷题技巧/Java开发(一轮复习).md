...
\##**偏向锁、轻量级锁和重量级锁**
synchronized的偏向锁、轻量级锁以及重量级锁是通过Java对象头实现的。博主在[Java对象大小内幕浅析](http://blog.csdn.net/u013256816/article/details/51008443)中提到了Java对象的内存布局分为：对象头、实例数据和对齐填充，而对象头又可以分为"Mark Word"和类型指针klass。"Mark Word"是关键，默认情况下，其存储对象的HashCode、分代年龄和锁标记位。
...
​		 数据量大的时候使用跳跃列表skiplist和哈希表hash_map结合实现，查找删除插入的时间复杂度都是O(logN)
...
​		[https://www.baidu.com/link](https://www.baidu.com/link?url=a1nIR9sc4lEPBYZIjnm6IUxkZ1WgVKqtWMKKbHLa7smmZt0PB_gJ4Ad0IcXLz999&wd=&eqid=815709f600005763000000065f39f595)
