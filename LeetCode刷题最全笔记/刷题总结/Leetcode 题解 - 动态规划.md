...
thumbnail: true
author: Kumi
date: 2020-4-28 22:20:51
icons: [fas fa-fire red, fas fa-star green]
cover: true
mathjax: true
top_meta: true
bottom_meta: true
headimg: https://cdn.jsdelivr.net/gh/kumi123/CDN/12.jpg
tags:
  - LeetCode刷题
  - 算法
categories:
...
#### 自己的算法 [参考](https://leetcode-cn.com/problems/house-robber-ii/solution/213-da-jia-jie-she-iidong-tai-gui-hua-jie-gou-hua-/)
...
class Solution:
    ...
    def helper(self,nums1):
        if len(nums1)==1:
            return nums1[0]
        res=[0 for i in range(len(nums1)+1)]
        res[1]=nums1[0]
        for i in range(2,len(nums1)+1):
            res[i]=max(res[i-2]+nums1[i-1],res[i-1])
        return res[-1]
...
class Solution:
    ...
    def helper(self,nums1):
        ...
        cur=nums1[0]
        for i in range(2,len(nums1)+1):
            cur,pre=max(pre+nums1[i-1],cur),cur
        return cur
```
...
class Solution {
    public int rob(int[] nums) {
        //这个关键点就是第一个位置和最后一个位置不能同时选，索引分成两个数列来求最大值
        if(nums.length==1) return nums[0];
        int[] nums_1=new int[nums.length-1];
        for(int i=0;i<nums.length-1;i++) nums_1[i]=nums[i];
        int[] nums_2=new int[nums.length-1];
        for(int i=1;i<nums.length;i++) nums_2[i-1]=nums[i];
        return Math.max(helper(nums_1),helper(nums_2));
    }
    public int helper(int[] nums1){
        if(nums1.length==1) return nums1[0];
        int pre=0,cur=nums1[0];
        for(int i=2;i<=nums1.length;i++){
            int tem=cur;
            cur=Math.max(pre+nums1[i-1],cur);
            pre=tem;
        }
        return cur;
...
#### [题解 从不优化的二维到优化过后的一维](https://leetcode-cn.com/problems/unique-paths/solution/dong-tai-gui-hua-by-powcai-2/)
...
##### 优化一 使用2n的空间
 ...
 因为递推方程$$dp[i][j]=dp[i-1][j]+dp[i][j-1]$$只是使用了当前行和前一行的数据，所以只需要两行空间来保存当前行和前一行

- 初始值 pre肯定是全为1，cur的话是第二行，需要进行计算 第一个数据是$$dp[1][1]$$其实就是cur[1]，明显是2 等价于cur[1]=cur[0]+pre[1] 因此cur的初始化只需要保证cur[0]就可以，其他的无所谓，但是为了好编码，全部赋值为1就可以
- 注意每一行cur进行更新完成之后,需要把cur的值赋值给pre，但是注意不能是pre=cur直接复制，因为后边如果是这样，cur和pre一致，cur的跟新之后pre也会发生变化
- 需要进行pre=cur.clone()(jave) 或者是pre=cur[:] (python)

```java
class Solution {
    public int uniquePaths(int m, int n) {
        if(m==1) return 1;//因为只有一行的时候啊输出的就是cur,所以直接复制cur全为1就是牛皮
        int[] pre=new int[n];
        int[] cur=new int[n];
        Arrays.fill(pre,1);
        Arrays.fill(cur,1);
...
- 第一，我们没有必要使用pre 可以直接使用cur单独就可以 为什么的呢？
- 之前的cur[j]=cur[j-1]+pre[j] pre[j] 代表着前一层相同列的路程个数，完全可以使用cur[j]来存储前一行
- 使用=左边的cur[j] 表示当前行的第j列
- 这样也不需要赋值了每一行完成后
...
class Solution {
    public int numberOfArithmeticSlices(int[] A) {
        ...
        //当满足形成等差数列的条件的时候，以A[i]为结尾的等差数列个数就会比A[i-1]结尾的数列个数多1，于是需要进行
        //dp[i]代表着以A[i]为结尾的等差数列个数
        //i=2开始,因为等差数列长度至少大于三个
...
#### [思路来源非常清晰](https://leetcode-cn.com/problems/integer-break/solution/bao-li-sou-suo-ji-yi-hua-sou-suo-dong-tai-gui-hua-/)
...
class Solution {
    public int integerBreak(int n) {
        if(n<=3) return n-1;//因为必须要拆分成至少两个数，数最小是2，2拆分成1*1 3差分成1*2 大于三的就用余数公式来求解
        int a=n/3,b=n%3;
        if(b==0) return (int)Math.pow(3,a);//别忘记要取int 因为pow默认返回的是double
...
class Solution {
    public int integerBreak(int n) {
        ...
        int[] dp=new int[n+1];
        dp[1]=0;
        //递推关系就是，dp[i]=max(dp[i],max((i-j)*j,j*dp[i-j])
        //其中j肯定从1到i-1 然后(i-j)*j 表示分成两个数  j*dp[i-j]表示分成大于两个数
        for(int i=2;i<=n;i++){
            for(int j=1;j<=i-1;j++){
...
![image-20210124171927165](https://cdn.jsdelivr.net/gh/kumi123/CDN//img2/image-20210124171927165.png)

```python
class Solution:
    def numSquares(self, n: int) -> int:
        from collections import deque#双端队列 左右两边都可以进，左右两边都可以出
        if n == 0: return 0
        queue = deque([n])#这个是首先需要弄进去一个值
        step = 0#每一步
        visited = set()#是否已经计算过，减少计算量
        while(queue):#层次遍历本质上
            step+=1#放在step的输出前边
            l=len(queue)
            for _ in range(l):
                tmp=queue.pop()#每一个节点
                #需要进行减去的平方数，并且从大到小减去比较合适，减去大的比较容易减少量
                for i in range(int(tmp**0.5),0,-1):
                    x=tmp-i**2
                    if(x==0):
                        return step
                    #如果已经在遍历过的节点了，就不需要再进行遍历了
                    if(x not in visited):
                        queue.appendleft(x)
                        visited.add(x)
        return step
...
##### [具体的思路](https://leetcode-cn.com/problems/perfect-squares/solution/hua-jie-suan-fa-279-wan-quan-ping-fang-shu-by-guan/)

题目描述：For example, given n = 12, return 3 because 12 = 4 + 4 + 4; given n = 13, return 2 because 13 = 4 + 9.

- 第一明确状态dp[i]代表和为i的最少平方数的个数
- 转移关系 dp[i] 可以由dp[i-j*j]+1 (i-j * j>=0)得到，因此dp[i] 应该是他们当中的最小值，需要循环进行比较就可以
- 这样转移方程也就出来$$ dp[i]=min(dp[i-j*j]+1 ) (i-j * j>=0) $$ 
...
class Solution:
    def numSquares(self, n: int) -> int:
        dp=[i for i in range(n+1)]
        for i in range(2,n+1):
            for j in range(1,int(i**(0.5))+1):#在这里
                dp[i]=min(dp[i],dp[i-j*j]+1)
        return dp[-1]
...
class Solution {
    public int numSquares(int n) {
        int[] res=new int[n+1];
        for(int i=0;i<=n;i++){
            res[i]=i;//初始化res[i]代表最多的组合数，肯定是i个1，慢慢的比较变化就可以 ,注意要保证res[0]=0
        }
        //res[i]代表的是和为i的最少的平方数
        for(int i=1;i<=n;i++){
...
##### [不错的题解](https://leetcode-cn.com/problems/maximum-length-of-pair-chain/solution/chuan-shang-yi-fu-wo-jiu-bu-ren-shi-ni-liao-lai--2/)

#### 直接使用贪心，区间排序就完事儿，想到射气球的那个
...
#### [思路直接无敌好不好](https://leetcode-cn.com/problems/wiggle-subsequence/solution/tan-xin-si-lu-qing-xi-er-zheng-que-de-ti-jie-by-lg/)
...
#### [好的思路](https://leetcode-cn.com/problems/longest-common-subsequence/solution/chao-xiang-xi-dong-tai-gui-hua-jie-fa-by-shi-wei-h/)
...
```java
    ...
    public int longestCommonSubsequence(String text1, String text2) {
        ...
        //注意dp[i][j] 代表的是s1当中的第i个到s2的第j个字符之间的最大公共子序列的个数
        //但是此时比较的应该是s1和s2当中下标为i-1和j-1两个东西
